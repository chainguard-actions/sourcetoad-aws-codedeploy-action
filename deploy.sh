...
if [ -z "$INPUT_ARCHIVE" ]; then
    ...
    # Build custom zip flags array safely to prevent word-splitting and metacharacter injection
    custom_zip_flags_array=()
...
safe_zip_filename=$(printf '%s' "$ZIP_FILENAME" | tr -d '\n\r')
